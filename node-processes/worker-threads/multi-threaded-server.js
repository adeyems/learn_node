...
function runWorker(workerData) {
    return new Promise((resolve, reject) => {
        ...
        const worker = new Worker(__dirname + "/sumOfPrimeWorker.js", {
            workerData,
        })
...
app.get("/sum-of-primes-with-threads", async (req, res) => {
    const startTime = new Date().getTime()
    const sum = await divideWorkAndGetSum()
        .then((values //values is an array containing all the resolved values
